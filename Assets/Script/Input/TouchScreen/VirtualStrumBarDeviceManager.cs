...
public class VirtualStrumBarDeviceManager : MonoBehaviour
{
    ...
    private void InitializeVirtualDevice()
    {
        ...
        InputSystem.RegisterLayout<VirtualTouchpadDevice>("VirtualTouchpad");
        ...
        Touchpad = InputSystem.devices.OfType<VirtualTouchpadDevice>().FirstOrDefault();

        // If no device was found, then it's safe to create a new one.
        if (Touchpad == null)
        {
            Touchpad = InputSystem.AddDevice<VirtualTouchpadDevice>("OnScreenTouch");
        }
    }
}
